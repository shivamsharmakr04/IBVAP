    ...
    <div className="p-4 md:p-6 space-y-6 max-w-[1920px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-slate-900/80 p-4 rounded-xl border border-slate-800">
        ...
        <button
          ...
          className="px-3 py-1.5 bg-slate-950 border border-slate-800 text-cyan-400 text-xs font-mono rounded-lg hover:border-cyan-500/50 flex items-center gap-1.5 self-start md:self-auto"
        >
      ...
      <div className="glass-panel p-4 rounded-xl border border-slate-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
        {/* Filter 1: Camera */}
        <div>
          <label className="block font-mono text-slate-400 mb-1">CCTV Camera / BOP</label>
          <select
        ...
        {/* Filter 2: Event Type */}
        <div>
          <label className="block font-mono text-slate-400 mb-1">AI Event Classification</label>
          <select
        ...
        {/* Filter 3: Severity */}
        <div>
          <label className="block font-mono text-slate-400 mb-1">Severity Level</label>
          <select
        ...
        <div>
          <label className="block font-mono text-slate-400 mb-1">ANPR Plate Search</label>
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-lg pl-8 pr-3 py-2 text-amber-400 font-mono focus:border-cyan-500 focus:outline-none"
            />
          </div>
        </div>
      </div>
              ...
              {filteredEvents.length === 0 ? (
                ...
                filteredEvents.map((evt) => (
                  <tr key={evt.id} className="hover:bg-slate-900/60 transition-colors">
                    ...
                    <td className="py-3 px-4 font-tactical font-bold text-slate-100">
                    ...
                    <td className="py-3 px-4">
                      <span className={`px-2 py-0.5 rounded text-[10px] font-bold border ${
                        evt.severity === 'CRITICAL' ? 'bg-rose-950 text-rose-400 border-rose-500/40' :
                        evt.severity === 'HIGH' ? 'bg-amber-950 text-amber-400 border-amber-500/40' :
                    ...
                    <td className="py-3 px-4">
                      {evt.plate_number ? (
                        <span className="text-amber-400 font-bold bg-slate-950 px-2 py-0.5 rounded border border-amber-500/30">
                        ...
                        <span className="text-slate-400">BBox: (x: {evt.bounding_box?.x || 120}, y: {evt.bounding_box?.y || 80})</span>
                      )}
                    </td>
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="px-2.5 py-1 bg-cyan-950 text-cyan-300 border border-cyan-500/30 hover:bg-cyan-900 rounded font-tactical font-bold text-xs"
                      >
                        View Snapshot
                      </button>
                    </td>
      ...
      {selectedEventModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
          <div className="glass-panel-glow max-w-2xl w-full rounded-2xl p-6 space-y-4 border border-cyan-500/40">
            <div className="flex items-center justify-between pb-2 border-b border-slate-800">
              ...
              <button onClick={() => setSelectedEventModal(null)} className="text-slate-400 hover:text-white font-mono text-sm">✕</button>
            </div>

            <div className="relative rounded-xl overflow-hidden bg-black border border-slate-800 h-64 flex items-center justify-center">
              <img
              ...
              />
              <div className="absolute top-4 left-4 bg-black/80 p-2 rounded border border-cyan-500/40 text-[11px] font-mono text-cyan-300">
                AI CONFIDENCE: {((selectedEventModal.confidence || 0.95) * 100).toFixed(1)}%
              </div>
            </div>

            <div className="grid grid-cols-2 gap-4 text-xs font-mono bg-slate-950 p-3 rounded-lg border border-slate-800">
              <div>
                <span className="text-slate-500">EVENT TYPE:</span>
                <p className="text-slate-200 font-bold">{selectedEventModal.event_type}</p>
              </div>
              <div>
                <span className="text-slate-500">TIMESTAMP:</span>
                <p className="text-slate-200">{new Date(selectedEventModal.timestamp).toLocaleString()}</p>
              </div>
            </div>
          </div>
        </div>
